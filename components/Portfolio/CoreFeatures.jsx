...
export default function CoreFeatures({ features }) {
  // If features prop is provided (from Appwrite), map it to expected shape.
  const list = Array.isArray(features) && features.length
    ? features.map((f, idx) => ({
        // prefer Appwrite $id if present or fallback to index-based id
        id: f.$id ?? f.id ?? idx,
        title: f.title ?? f.titleText ?? `Feature ${idx + 1}`,
        description: f.description ?? f.desc ?? f.content ?? "",
      }))
    : FALLBACK_FEATURES;
  ...
  return (
    <div className="custom-container 2xl-custom:w-[1580px] 2xl-custom:mx-auto">
      <div className="flex flex-col justify-center items-center gap-4">
        ...
        <p className="text-paragraph text-[18px] font-[400] leading-[24px] max-w-2xl text-center">
          Designed to make SRMH knowledge and support more accessible, inclusive,
          and confidential.
        </p>
